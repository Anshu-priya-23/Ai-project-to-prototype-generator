...
import com.enterprise.ai.messaging.PrototypeSpecification;
...
public class PrototypeGenerator {
    ...
    public PrototypeGenerator(ChatLanguageModel model, ObjectMapper objectMapper,
    ...
    public PrototypeResult generate(PrototypeRequest request) {
        try {
            ...
            validate(spec);
            return new PrototypeResult(request.projectId(), "COMPLETED",
                    objectMapper.writeValueAsString(spec), null);
        } catch (Exception exception) {
            return new PrototypeResult(request.projectId(), "FAILED", null, safeMessage(exception));
        }
    }
    ...
    private boolean isEmpty(java.util.List<?> values) { return values == null || values.isEmpty(); }
